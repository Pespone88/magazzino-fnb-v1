...
export function mapOrdersError(error: Exclude<QueryError,null>): Error {
  ...
  if (context.includes('supplier_receipts_extra_explained')) return new Error('Se inserisci un importo extra, specifica anche la spiegazione.')
  if (context.includes('supplier_receipt_lines_wrong_item_coherent')) return new Error('Per un articolo errato indica correttamente se viene rifiutato o accettato come altra referenza.')
  if (context.includes('supplier_receipt_lines_missing_coherent')) return new Error('Un articolo mancante deve avere quantità ricevuta e accettata pari a zero.')
  if (context.includes('supplier_receipt_lines_unbilled_coherent')) return new Error('Una riga non fatturata deve avere quantità documentata pari a zero.')
  if (context.includes('supplier_receipt_lines_outcome_coherent')) return new Error('Quantità, esito e gestione della riga non sono coerenti.')
  ...
  if (context.includes('Order line is not receivable')) return new Error('Questa riga non può ricevere altra merce nello stato attuale.')
