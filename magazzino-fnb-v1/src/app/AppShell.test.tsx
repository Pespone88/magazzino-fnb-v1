...
describe('AppShell', () => {
  ...
  it('opens Ordini for the active store and reloads it when store changes', async () => {
    const user = userEvent.setup()
    render(
      <AppShell
        context={context}
    ...
    await user.selectOptions(screen.getByLabelText('Store attivo'), 'store-nonna-titti')
    expect(ordersGateway.listNeedCandidates).toHaveBeenCalledWith('store-nonna-titti')
